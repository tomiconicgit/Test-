...
export class Controls {
  ...
  isPositionValid(pos) {
    ...
    const checks = [[0,0,0],[0,this.playerHeight/2,0],[0,this.playerHeight,0]];
    for (let x=-half;x<=half;x+=this.playerWidth){
      for (let z=-half;z<=half;z+=this.playerWidth){
  ...
  tryMove(delta, feet) {
    if (!this.isPositionValid(feet.clone().add(delta))) return false;
    feet.add(delta); return true;
  }
