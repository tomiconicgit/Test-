...
export class CameraRig {
  constructor() {
    ...
    // Keep colors from blowing out
    this.renderer.outputEncoding = THREE.sRGBEncoding;          // r128
    this.renderer.toneMapping = THREE.ACESFilmicToneMapping;
    this.renderer.toneMappingExposure = 0.9;                    // ↓ for more headroom
    this.renderer.physicallyCorrectLights = true;

    document.body.appendChild(this.renderer.domElement);
    const cv = this.renderer.domElement;
    cv.style.position = 'fixed'; cv.style.inset = '0'; cv.style.zIndex = '0';
    cv.style.display = 'block'; cv.style.pointerEvents = 'none';

    this.touchStartX = 0; this.touchStartY = 0;
    this.lon = -90; this.lat = 0; this.phi = 0; this.theta = 0;
    this._mouseDragging = false; this._mx = 0; this._my = 0;
  ...
  onTouchStart(e){ const t=e.touches[0]?.target; if(t&&(t.closest('#tower-ui')||t.closest('#joystick-container'))) return;
    e.preventDefault(); this.touchStartX=e.touches[0].clientX; this.touchStartY=e.touches[0].clientY; }
  onTouchMove(e){ const t=e.touches[0]?.target; if(t&&(t.closest('#tower-ui')||t.closest('#joystick-container'))) return;
  ...
  onMouseDown(e){ if(e.target.closest && (e.target.closest('#tower-ui')||e.target.closest('#joystick-container'))) return;
    this._mouseDragging=true; this._mx=e.clientX; this._my=e.clientY; }
  onMouseMove(e){ if(!this._mouseDragging) return;
    ...
    this.lon += (-dx)*0.2; this.lat += (-dy)*0.2; this.lat=Math.max(-85,Math.min(85,this.lat));
