...
export class PlacementController {
    ...
    update(world, player, activeItem, input) {
        ...
        const propIntersects = this.raycaster.intersectObjects(world.props, false); // Assuming props are on world object
    ...
    _placeProp(world, activeItem) {
        ...
        this.scene.add(newProp); world.props.push(newProp);
    }

    remove(world) {
        this.raycaster.setFromCamera({ x: 0, y: 0 }, this.camera); const intersects = this.raycaster.intersectObjects(world.props);
        if (intersects.length > 0) { const obj = intersects[0].object; if(this.snapTarget === obj) { this.isSnapping = false; this.snapTarget = null; } this.scene.remove(obj); world.props.splice(world.props.indexOf(obj), 1); if (obj.geometry) obj.geometry.dispose(); }
